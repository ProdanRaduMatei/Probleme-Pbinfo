...
inline bool Valid(int j, int i, int aSum, int bSum) {
    if ((S ^ sta[j - 1] ^ bSum ^ dra[i + 1]) == 0 && sTotal==suma_st[j-1] + suma_dr[i+1]+(sumb_st[i] - sumb_st[j - 1]) && sPP==app_st[j-1] + app_dr[i+1]+(bpp_st[i] - bpp_st[j - 1]))
            return true;
    if ((S ^ stb[j - 1] ^ aSum ^ drb[i + 1]) == 0 && sTotal==sumb_st[j-1] + sumb_dr[i+1]+(suma_st[i] - suma_st[j - 1]) && sPP==bpp_st[j-1] + bpp_dr[i+1]+(app_st[i] - app_st[j - 1]))
            return true;
    return false;
}
